...
const formatDate = (dateString: string | null | undefined): string => {
  ...
  const ClientDocuments = () => {
    ...
    const fetchData = useCallback(async () => {
      ...
      const [uploadedResult, sharedResult, chatResult] = await Promise.all([
        getDocuments(),
        getSharedDocuments(),
        getChatDocuments(), // Ensure this is included
      ]);

      setUploadedDocs(uploadedResult);
      setSharedWithMeDocs(sharedResult);
      setChatDocs(chatResult);
      ...
      chatResult.forEach((doc) => combined.set(doc.id, doc));
      setAllDocs(
        Array.from(combined.values()).sort(
      ...
      // Fetch profiles of users who shared documents or sent chat documents
      const profileIds = new Set<string>();
      sharedResult.forEach((doc) => {
        if (doc.user_id) profileIds.add(doc.user_id);
      });
      chatResult.forEach((doc) => {
        if (doc.user_id) profileIds.add(doc.user_id); // Sender of chat document
      });

      if (profileIds.size > 0) {
        setIsLoadingProfiles(true);
        const profiles = await getUserProfiles(Array.from(profileIds));
          ...
          <div className="space-y-4">
            {docs.map((doc) => (
              <ClientDocumentRow
                key={doc.id}
                document={doc}
                sharerProfile={sharerProfiles.get(doc.user_id)}
                isOwner={doc.user_id === currentUserId && !doc.isChatDocument} // Chat docs are not "owned" for deletion
        ...
        <DashboardLayout userType="user">
          <div className="grid gap-4">
            ...
            <p className="text-muted-foreground">
              View documents shared with you, your uploads, and chat attachments.
            </p>
          </div>
            ...
            <Tabs defaultValue="all" className="w-full">
              <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
                <TabsList>
                  ...
                  <TabsTrigger value="chat">Chat Documents</TabsTrigger> {/* New Tab */}
                </TabsList>
              ...
              <TabsContent value="all">
                <Card>
                  ...
                  <CardContent>{renderDocumentList(allDocs)}</CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="shared">
                <Card>
                  ...
                  <CardContent>{renderDocumentList(sharedWithMeDocs)}</CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="uploaded">
                <Card>
                  ...
                  <CardContent>{renderDocumentList(uploadedDocs)}</CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="chat">
                <Card>
                  <CardHeader>
                    <CardTitle>Chat Documents</CardTitle>
                    <CardDescription>Attachments shared in your chats</CardDescription>
                  </CardHeader>
                  <CardContent>{renderDocumentList(chatDocs)}</CardContent>
                </Card>
              </TabsContent>
            </Tabs>
          </div>
...
const ClientDocumentRow = ({
  ...
  onShare,
  onDelete,
}: ClientDocumentRowProps) => {
  const sharerName = sharerProfile?.full_name || 'Anonymous User';
  ...
  return (
    <div className="flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 transition-colors gap-2">
            ...
            {document.isChatDocument ? (
              <>
                <div className="flex items-center gap-1">
                  ...
                  <span>From Chat by {sharerName}</span>
                </div>
                <span>•</span>
              </>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {isOwner && !document.isChatDocument && (
          <>
